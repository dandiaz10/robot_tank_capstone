...
class RobotTankServer(object):
  def __init__(self, debug):
    ...
    self.done = False # when is false the comunitcation is stablished
...
#return the highest prioriy direction when multiple keys are pressed
  ...
  # increase all the directions priority exept for the current direction
  # current direction priority is set to 0
...
# read the pressed keys and update the priority
# argumeent e is a keyboard event read
  def on_keyboard_event(self, e):
    if e is not None:
      if e['key'] == '+w' and e['is_up']: # e['is_up'] == True means the key was pressed
